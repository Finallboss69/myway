...
import { STAFF, elapsedMinutes } from "@/data/mock";
...
function ItemStatusChip({ status }: { status: ItemStatus }) {
	const cls = clsx(
...
function OrderCard({ order }: { order: Order }) {
	...
	function handleItemToggle(itemId: string, currentStatus: ItemStatus) {
		...
		updateItemStatus(order.id, itemId, next[currentStatus] ?? currentStatus);
	}

	function handleAllStart() {
		order.items
			.filter((i) => i.status === "pending")
			.forEach((i) => updateItemStatus(order.id, i.id, "preparing"));
	}
	...
	function handleAllDelivered() {
		order.items
			...
			.forEach((i) => updateItemStatus(order.id, i.id, "delivered"));
	}

	// Station derived from first item with a station (mock has "station" but our store doesn't — use generic)
	const station = null;
			...
			<div className="flex items-start justify-between px-4 pt-4 pb-3 border-b border-surface-3 bg-surface-2/40">
				<div className="flex items-baseline gap-3">
					...
					<div className="flex flex-col gap-1.5 mt-auto pb-1">
						...
						<span
							className={clsx(
								"inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-display font-bold uppercase tracking-wider border",
								"bg-surface-3 text-ink-secondary border-surface-4",
							)}
						>
...
export default function KitchenKDSPage() {
	...
	const anaStaff = STAFF.find((s) => s.role === "kitchen");

	// Map orders to derived status for display
			...
			<header className="sticky top-0 z-40 bg-surface-1/95 backdrop-blur-md border-b border-surface-3 shadow-[0_1px_0_rgba(255,255,255,0.04)]">
				<div className="flex items-center px-6 py-3 gap-4">
					...
					<div className="absolute left-1/2 -translate-x-1/2 flex flex-col items-center">
						<span className="font-kds text-4xl leading-none text-ink-secondary tracking-[0.1em]" suppressHydrationWarning>
							{currentTime}
						</span>
