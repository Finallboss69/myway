...
import { INGREDIENTS, PRODUCTS, formatCurrency } from "@/data/mock";
...
function getStatus(ing: (typeof INGREDIENTS)[number]): IngStatus {
	if (ing.stockCurrent <= ing.alertThreshold * 0.6) return "critical";
	if (ing.stockCurrent <= ing.alertThreshold) return "low";
	return "ok";
}

function stockPct(ing: (typeof INGREDIENTS)[number]): number {
	...
	return (
		<button
			...
			className={clsx(
				...
				on ? "bg-brand-500 shadow-gold-sm" : "bg-surface-4",
			)}
		>
			<div
				className={clsx(
...
export default function BarStockPage() {
	...
	const [productToggles, setProductToggles] = useState<Record<string, boolean>>(
		() => Object.fromEntries(PRODUCTS.map((p) => [p.id, p.isAvailable])),
	);

	const barProducts = PRODUCTS.filter(
		(p) => p.target === "bar" && !p.isPoolChip,
	);
	const poolChips = PRODUCTS.filter((p) => p.isPoolChip);
	...
	function toggleProduct(id: string) {
		setProductToggles((prev) => ({ ...prev, [id]: !prev[id] }));
	}
		...
		<div className="flex min-h-screen bg-surface-0">
			...
			<div className="flex-1 flex flex-col min-w-0">
				...
				<div className="sticky top-0 z-20 flex items-center justify-between px-8 py-5 border-b border-surface-3 bg-surface-1/95 backdrop-blur-md">
					<div>
						...
						<p className="font-body text-sm text-ink-tertiary ml-11">
							Ingredientes y disponibilidad de productos ·{" "}
				...
				<div className="flex-1 p-8 space-y-6">
					...
					{criticalIngredients.length > 0 && (
						<div
							className="rounded-2xl p-4 flex items-start gap-4 animate-fade-in"
							style={{
					...
					<div className="card rounded-2xl overflow-hidden">
						<div className="px-5 py-4 border-b border-surface-3 flex items-center justify-between bg-surface-2/20">
							...
							<div className="flex items-center gap-3">
								...
								<button className="btn-ghost text-xs flex items-center gap-1.5 py-1">
									<RefreshCw size={11} />
									Actualizar
								</button>
							</div>
											...
											>
												...
												<td className="px-5 py-3.5">
													<div className="flex items-center justify-end gap-2">
														<button className="btn-ghost text-[11px] px-2.5 py-1.5 rounded-lg">
															<RefreshCw size={11} />
															Ajustar
														</button>
													</div>
					...
					<div className="card rounded-2xl overflow-hidden">
						<div className="px-5 py-4 border-b border-surface-3 flex items-center justify-between bg-surface-2/20">
							...
							<span className="text-ink-tertiary font-body text-xs">
								{barProducts.length + poolChips.length} productos
							</span>
						</div>
						...
						<div className="p-5 space-y-4">
							...
							<div>
								<div className="flex items-center gap-2 mb-3">
									<Wine size={13} className="text-ink-tertiary" />
									<h3 className="font-display text-[10px] uppercase tracking-widest text-ink-tertiary font-bold">
										Bebidas &amp; Tragos
									</h3>
								</div>
									...
									{barProducts.map((prod) => (
										<div
											...
											className={clsx(
												...
												productToggles[prod.id]
													? "bg-surface-2/50 border-surface-3 hover:border-surface-4"
													: "bg-surface-2/20 border-surface-3/40 opacity-60",
											)}
										>
											<div className="flex flex-col min-w-0 flex-1 mr-3">
												<span className="font-display text-sm font-semibold text-ink-primary truncate">
													{prod.name}
												</span>
												<span className="text-ink-tertiary text-xs font-body truncate">
													{prod.description}
												</span>
											</div>
											<div className="flex items-center gap-2.5 shrink-0">
												...
												<ToggleSwitch
													on={productToggles[prod.id] ?? prod.isAvailable}
								...
								<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5">
									{poolChips.map((prod) => (
										<div
											...
											style={{
												background: productToggles[prod.id]
													? "rgba(245,158,11,0.06)"
													: "rgba(245,158,11,0.02)",
												borderColor: productToggles[prod.id]
