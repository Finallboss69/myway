...
export default function TableDetailPage({
	...
	const table =
		tables.find((t) => t.id === tableId) ?? tables.find((t) => t.id === "t2")!;
	const activeOrders = orders.filter(o => o.tableId === table.id && o.status !== "closed");
	const cart = carts[table.id] ?? [];
	...
	// Aggregate all items across active orders for display
	...
	// Elapsed from the oldest active order
	const elapsed =
	...
	const handleAddToCart = (product: (typeof PRODUCTS)[number]) => {
		...
		addToCart(table.id, item);
	};

	const handleSubmitCart = () => {
		if (!cart.length) return;
		submitCart(table.id, cashier.name);
		showToast(
			`${cart.length} ítem${cart.length !== 1 ? "s" : ""} enviado${cart.length !== 1 ? "s" : ""} a cocina/bar`,
		);
	};

	const handleCloseTable = (method: PaymentMethod) => {
		closeTable(table.id, method);
			...
			>
				...
				>
									...
									{cart.map((item) => (
										...
										>
											...
											<button
												...
												onClick={() => removeFromCart(table.id, item.productId)}
											>
												<X size={12} />
											</button>
										</div>
					...
					>
						...
						{/* Send cart shortcut (if cart has items) */}
						{cart.length > 0 && (
						...
						{allOrderItems.length > 0 && (
							<div style={{ padding: "14px 20px 24px" }}>
								...
								<button
									...
									style={{ paddingTop: 14, paddingBottom: 14, fontSize: 12 }}
									onClick={() => handleCloseTable("cash")}
								>
