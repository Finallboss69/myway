	...
	return (
		<aside className="sidebar top-accent relative">
			...
			<div className="px-3 pb-4 pt-3 border-t border-surface-3 flex-shrink-0 space-y-2">
				{/* Notifications */}
				<button className="sidebar-item w-full relative">
					<Bell size={15} className="flex-shrink-0" />
					<span className="flex-1 text-left">Notificaciones</span>
					{unreadCount > 0 && (
						<span className="flex-shrink-0 min-w-[18px] h-[18px] rounded-full bg-brand-500 text-surface-0 font-display text-[9px] font-700 flex items-center justify-center px-1">
							{unreadCount}
						</span>
					)}
				</button>
				...
				<div className="flex items-center gap-2.5 px-3 py-2.5 rounded-lg bg-surface-2 border border-surface-3">
					...
					<button className="text-ink-tertiary hover:text-status-cancelled transition-colors flex-shrink-0 p-1">
						<LogOut size={13} />
					</button>
				</div>
			</div>
